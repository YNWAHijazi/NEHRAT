  ...
  return invitations
    ...
    .map((inv) => {
      ...
      return {
        ...
        owedEn: isL3 ? 'EMS Readiness Declaration' : 'Complete participation details',
        owedAr: isL3 ? 'إقرار جاهزية خدمات الطوارئ الطبية' : 'إكمال تفاصيل المشاركة',
        byLabelEn: 'Due',
        byLabelAr: 'المهلة',
        byEn: fileBy(inv),
        byAr: `⁦${fileBy(inv)}⁩`,
        ...
        ...(closed
          ? { doneChipEn: 'Removed', doneChipAr: 'أُزيلت' }
          : done && inv.signedAt
  ...
  return (
    <div>
      ...
      <div data-region="outstanding" style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBlockEnd: 52 }}>
        {outstanding.length === 0 ? (
          <div style={{ padding: '24px 26px', border: '1px dashed var(--line)', borderRadius: 16, fontSize: 15, color: 'var(--muted)' }}>
            <L en="No pending tasks." ar="لا مهام معلّقة." />
          </div>
        ) : null}
        {outstanding.map((e) => (
      ...
      <div data-region="complete" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {complete.length === 0 ? (
          <div style={{ padding: '24px 26px', border: '1px dashed var(--line)', borderRadius: 16, fontSize: 15, color: 'var(--muted)' }}>
            <L en="No completed tasks yet." ar="لا مهام مكتملة بعد." />
          </div>
        ) : null}
        {complete.map((e) => (
          <Link key={e.key} href={e.href} data-stack="" style={{ paddingBlock: '23px', paddingInlineStart: '26px', paddingInlineEnd: '27px', background: 'var(--surface2)', borderInlineStart: '3px solid var(--brand)', borderRadius: 16, display: 'grid', gridTemplateColumns: 'minmax(200px,1.6fr) 1fr auto', gap: 20, alignItems: 'center', color: 'var(--ink)' }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-.015em', marginBlockEnd: 5 }}>
                <L en={e.nameEn} ar={e.nameAr} />
              </div>
