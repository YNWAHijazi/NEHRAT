...
**0. The reference prototypes cannot resolve a minimum condition from the assessment, and you must not port that.** Seven of the ten minimum conditions carry `derived: false` in `Organizer Journey.dc.html` — `att3`, `club`, `run21`, `tri`, `open`, `combat`, `motor`. They are manual checkboxes because the prototype has no structured input to derive them from. Of the two that do derive, `run` **over-fires**: it triggers on domain 2's top option, which bundles running with cycling, triathlon, open-water, combat sport and motor racing, so a boxing match trips "organized running event".

The consequence is wider than any single field: **domain 2's answer cannot resolve any discipline floor.** Four Level 3 conditions — triathlon, open water, combat sport, motor racing — are all manual, so an event answering the assessment correctly returns Level 2 unless someone remembers a checkbox.

The build captures **event disciplines as a structured list**, and every discipline condition derives from it. Attendance is captured as a number and course distance as a required field on running events. **An unset input returns "incomplete" with the field named — never a level.** A silently wrong level is the failure this platform exists to prevent; a loud refusal is not.

**1. The event level is derived, never chosen.** Nine domains scored 0–2, total 0–18, bands 0–5 / 6–11 / 12–18, plus ten minimum conditions attached to event types, disciplines and attendance figures. **The final level is the higher of the score band and the minimum condition.** The UI must report both results and which one governed. If any surface lets a user pick a level, the build is wrong.
