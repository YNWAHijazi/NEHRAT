...
8. **The two issues order requirements 16 and 17 differently.** English has 16 major-incident, 17 insurance; the Arabic issue reverses them. Anything pairing the two issues **by index rather than by content** silently swaps both titles — an Arabic reader sees the insurance title on the major-incident row. Pair by content, always, and treat index alignment between the two issues as unsafe generally.
9. **Level 1 and the compliance form** — Protocol §8.1 says Annex C and a full plan are *not routinely required* at Level 1; Annex C's own header says a Level 1 organizer completes it *only when requested*. The build follows Annex C.
