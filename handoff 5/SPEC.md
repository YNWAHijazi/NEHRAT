...
Then a set of **minimum conditions** applies. **The two issues of Annex A disagree on how many there are, and the build carries a reconciliation, not the regulation.** The English Part D has nine rows including *nightclub or dance venue 1,000+* and treats the recurring venue as an annual-assessment duty with no floor. The Arabic Part D also has nine, including *recurring venue 1,000+ → Level 2* and omitting the nightclub row. The build carries the **union of both — ten** — because a union can only raise a level and never lower one. Every row is tagged with the issue that carries it, and the divergence is for the Ministry to settle. Example: 20,000 or more people present at the same time forces Level 3 regardless of score. **The final level is the higher of the two results.** Show both and show which one governed; an organizer who scored 9 but lands at Level 3 must be able to see why.
...
## 2b. Arabic is quoted regulation, and two words are not interchangeable

The Arabic issues use **التأهب** for *preparedness* — the mass-gathering instrument — and reserve **الجاهزية** for *readiness* in the cardiac-arrest and first-response sense. Earlier versions of this glossary conflated them. They are not synonyms and the distinction is regulatory.

The three outcomes, verbatim from the Arabic compliance form's Ministry-use block:

- تم استلام التقديم، لكنه غير مكتمل
- يلزم تقديم معلومات إضافية أو إجراء تعديلات
- تم استيفاء متطلبات التأهب الصحي والطبي

The plan is **خطة التأهب الصحي والطبي للفعالية**.

**Where an Arabic issue of a document exists, its wording is the wording.** Not a translation of the English, and not a translation regenerated later. The reference prototypes predate this rule and their Arabic assessment strings are translations — they are being re-issued from the Arabic NEHRAT. Until that lands, treat every Arabic assessment string in the prototypes as provisional and the Arabic issue as authoritative.

---

## 2c. Undecided, and not to be resolved by a build

**Who the platform owner can see.** The roadmap says platform activity is counts only, with no organizer, account, event or patient named and no filtering to one organization's behaviour. The product specification says the owner layer shall support full tenant visibility including records, and describes an assistant answering record-level queries. That is a direct contradiction with privacy consequences for Lebanese health records, and it is a Ministry decision rather than a design one.

**Until it is decided, counts only.** Build the narrower reading. It is reversible; the wider one is not.

**Commercial capability versus commercial content.** No commercial content renders in the Lebanon tenant — no vendor directory, no purchase links, no fees, no sponsored listings. The product specification separately requires the *architecture* to support those behind feature flags, because the platform is licensed rather than bespoke. Capability flagged off and content never rendered are different things: build the flags, ship them off, render nothing.
...
## 12. Carried by the sources, absent from this summary

Found by reading the regulation against this document. Each is real and none is built.

- **An AI layer is a required architecture component.** Three role-scoped assistants, bilingual, grounded in the configured regulatory sources, with human-confirmation guardrails.
- **Multi-tenant white-label architecture.** The platform is licensed to the Ministry, not built for it alone. Affects the data model, not just deployment.
- **Short-notice events.** Expedited submission that waives no requirement. The Ministry console flags them; no organizer-side screen exists anywhere.
- **Assessment records its tool version.** Every assessment must record which version of the instrument scored it. Assessments are versioned; the instrument is not.
- **Data minimisation is event-side too.** Aggregate data and no unnecessary personal information applies to the post-event report, not only to the facility incident report.
- **Annex A Part A is fifteen fields.** Opening and closing times, the participants / spectators / staff split, previous edition, licensed capacity. The build captures four.
- **The facility immediate-response procedure is eight steps, and there are ten Ministry configuration powers.** Earlier documents said seven and nine.
- **The Guidance's fourteen-section template is not the Protocol's sixteen mandatory items.** Present the sixteen as the checklist; the template may be offered as non-binding structure, labelled as creating no requirements.
